...
def gate_signup():
    ...
    try:
        ...
        # ── SHEET3 SYNC (ZAPIER_USERS_WEBHOOK_URL) ────────────────────────────
        try:
            users_webhook = os.environ.get('ZAPIER_USERS_WEBHOOK_URL', '')
            if users_webhook:
                http_requests.post(
                    users_webhook,
                    json={
                ...
                logger.info(f'[USERS] Synced to Sheet3: {email}')
        except Exception as e:
            logger.warning(f'[USERS] Sheet3 sync failed: {e}')
        # ── END SHEET3 ────────────────────────────────────────────────────────
...
# ── MY ACCOUNT — login + account dashboard ──────────────────────────────────
...
def send_login_code():
    ...
    stored = f'{code}|{expiry}'

    try:
        cache_set(f'login_code_{email}', stored)
    except Exception:
        pass
    if not hasattr(app, '_login_codes'):
        app._login_codes = {}
    app._login_codes[email] = stored
...
def verify_login_code():
    ...
    stored = None
    try:
        stored = cache_get(f'login_code_{email}')
    except Exception:
        pass
    ...
    SUB_FILE = '/var/data/mora_assists_subscribers.csv'
    if os.path.exists(SUB_FILE):
        with open(SUB_FILE) as f:
            for row in _csv.DictReader(f):
                if row.get('email', '').lower() == email:
                    mora_status = row.get('status', 'none')
                    trial_ends  = row.get('trial_ends_at', '')[:10]
                    name        = row.get('name', '')
                    if mora_status in ['active', 'trial']:
                        plan = mora_status
                    break

    if not name:
        FREE_FILE = '/var/data/email_subscribers.csv'
    ...
    # Expire used code
    try:
        cache_set(f'login_code_{email}', '')
        if hasattr(app, '_login_codes'):
            app._login_codes.pop(email, None)
    except Exception:
        pass

    logger.info(f'[LOGIN] Verified: {email} plan={plan}')
    return jsonify({
        'success':     True,
        'email':       email,
        'name':        name,
